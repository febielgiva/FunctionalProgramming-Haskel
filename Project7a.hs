...
instance Show Game where
  ...
  show = concat . surround "\n     " . makeOutputStrings  
    ...
    -- Construct the output as an List of Strings by combining the
...
-- If the game is over because some player has 3 in a row, return:
--   Just 'X': x wins
--   Just 'O': o wins
-- If the game is over because the board is full, return 
--   Just '-': tie game
...
gameOver game = 
  ...
  -- If we can find 3 identical player values in a row, that player wins.
  where 
...
minimax game = 
  ...
  selectByPlayer onXMove onOMove =          
...
next game 
  | -- If the game is starting or over, start a new game
    count e game == 9 || isJust (gameOver game) = start
